...
namespace EPAM.Tests.task06_modifiers
{
    public abstract class BaseClass
    {
        ...
        {
            return GetDefaultDescription() + this.GetClassSymbol();
        }
    ...
    internal class DerivedClassA : BaseClass
    {
        ...
        protected override string GetClassSymbol()
        {
            return "A";
        }
    }
    ...
    internal class DerivedClassB : DerivedClassA
    {
        ...
        public new string GetCode()
        {
            return DerivedClassImpl.GetCode();
        }
    ...
    public class Program
    {
        ...
        private static void VerifyAccessibilityLevels(Type x935, string x807, Type x742, string x458, Type x671)
        {
            TestType(x935, t => t.IsPublic, s1);
            TestType(x742, t => t.IsPublic == false, s1);
            TestType(x671, t => !t.IsPublic, s1);
            TestType(x935, t => t.GetMethods().FirstOrDefault(m => m.Name == x807) == null, string.Format(s4, x807));
            TestType(x935, t => t.GetMethods().FirstOrDefault(m => m.Name == x458) == null, string.Format(s4, x458));
        }

        private static void TestType(Type t, Func<Type, bool> f, string m)
        {
            Debug.Assert(f(t), string.Format("{0}{1}", t.Name, m));
        }

        const string s1 = " has wrong accessibility modifier.";
        const string s2 = "GetDefaultDescription";
        const string s3 = "GetClassSymbol";
        const string s4 = "::{0} has wrong accessibility level";
    }
}